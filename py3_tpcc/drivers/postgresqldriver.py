"""
PostgreSQL TPC-C Database Driver

Implements the `AbstractDriver` interface to execute TPC-C transactions
...
TXN_QUERIES = {
    ...
    "NEW_ORDER": {
        ...
        "createOrder": "INSERT INTO ORDERS (O_ID, O_D_ID, O_W_ID, O_C_ID, O_ENTRY_D, O_CARRIER_ID, O_OL_CNT, O_ALL_LOCAL) VALUES (%s, %s, %s, %s, %s, %s, %s, %s::integer)",
...
class PostgresqlDriver(AbstractDriver):
    """
    Concrete implementation of the PostgreSQL database driver.
    """
    ...
    DEFAULT_CONFIG = {
        ...
        "isolation-level": ("The transaction isolation-level for the PostgreSQL database. Valid values are `default`, `read_uncommitted`, `read_committed`, `repeatable_read`, `serializable`", "default"),
        "reset": ("Reset the database schema and tables before starting", False)
    }
    
    def __init__(self, name: str, ddl: str):
        super().__init__("postgresql", ddl)
    ...
    def load_config(self, config: Any) -> Optional[Dict[str, Any]]:
        ...
        # Unwrap tuples from DEFAULT_CONFIG if they weren't overridden properly
    ...
    def connect(self) -> None:
        ...
        if psycopg is None:
            raise ConnectionError("The psycopg library is not installed or failed to load. Cannot connect to PostgreSQL.")
            
        logger.info("Connecting to database: %s", self.config.get("database", "tpcc"))
        try:
            self.conn = psycopg.connect(
                dbname=self.config.get("database", "tpcc"),
        ...
        isolation_level = self.config.get("isolation-level", "default").lower()

        if isolation_level == "read_uncommitted":
            self.conn.isolation_level = psycopg.IsolationLevel.READ_UNCOMMITTED
            ...
            logger.info("Isolation level set to READ COMMITTED.")
        elif isolation_level == "repeatable_read":
            self.conn.isolation_level = psycopg.IsolationLevel.REPEATABLE_READ
            logger.info("Isolation level set to REPEATABLE READ.")
        elif isolation_level == "serializable":
            self.conn.isolation_level = psycopg.IsolationLevel.SERIALIZABLE
            logger.info("Isolation level set to SERIALIZABLE.")
        elif isolation_level == "default":
            # Setting to None allows driver to use database default
            self.conn.isolation_level = None
            logger.info("Isolation level set to DEFAULT (PostgreSQL's default).")
        else:
            logger.warning(
                f"Unrecognized isolation level '{isolation_level}'. "
                "Valid values are `default`, `read_uncommitted`, `read_committed`, "
                "`repeatable_read`, `serializable`. Using PostgreSQL default."
            )
    ...
    def reset(self) -> None:
        ...
        db_name = self.config.get("database", "tpcc")
        logger.info("Dropping all tables in database '%s'", db_name)
    ...
    def load_ddl(self) -> None:
        ...
        db_name = self.config.get("database", "tpcc")
        logger.info("Restoring schema definitions in database '%s'", db_name)
    ...
    def load_tuples(self, table_name: str, tuples: List[Any]) -> None:
        ...
        # Disable triggers/constraints for faster copy/insert
        ...
        logger.debug("Loaded %d tuples for table_name %s", len(tuples), table_name)
    ...
    def load_finish(self) -> None:
        ...
        logger.info("Committing changes to database after load")
        self.conn.commit()
            ...
            try:
                ...
                for i in range(len(i_ids)):
                    ...
                    if stockInfo is None or len(stockInfo) == 0:
                        logger.warning("No STOCK record for (ol_i_id=%d, ol_supply_w_id=%d)", ol_i_id, ol_supply_w_id)
                        continue
